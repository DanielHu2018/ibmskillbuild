"""
NARRA — Email Alert Service (Python / Server-Side)
...
logger = logging.getLogger("narra.email")
...
def _alert_subject(alert_type: AlertType, symbol: str) -> str:
    labels = {
        AlertType.SELL_TARGET_HIT:   f"🎯 NARRA Alert: {symbol} Reached Sell Target",
        AlertType.STOP_LOSS_HIT:     f"🛑 NARRA CRITICAL: {symbol} Stop Loss Triggered",
        AlertType.TRAILING_STOP_HIT: f"⚠️ NARRA Alert: {symbol} Trailing Stop Hit",
        AlertType.BULL_TARGET_HIT:   f"🚀 NARRA Alert: {symbol} Bull Target Reached",
        AlertType.DAILY_SUMMARY:     f"📊 NARRA Daily Portfolio Summary",
    }
    return labels.get(alert_type, f"NARRA Alert: {symbol}")
        ...
        <table width="600" cellpadding="0" cellspacing="0" style="background:#1e293b;border-radius:16px;overflow:hidden;border:1px solid #334155;">
          ...
          <tr>
            <td style="background:{color};padding:24px 32px;">
              <table width="100%" cellpadding="0" cellspacing="0">
                <tr>
                  <td>
                    <p style="margin:0;color:rgba(255,255,255,0.8);font-size:12px;letter-spacing:2px;text-transform:uppercase;">NARRA — Narrative Risk Reasoning Agent</p>
          ...
          <tr>
            <td style="padding:16px 32px 0;background:#0f172a;">
              <div style="background:{color}1a;border-radius:10px;padding:20px;border:1px solid {color}44;">
                <p style="margin:0;color:{color};font-size:12px;text-transform:uppercase;letter-spacing:1px;font-weight:600;">NARRA Recommendation</p>
          ...
          <tr>
            <td style="padding:24px 32px;background:#0f172a;text-align:center;">
              <p style="margin:0;color:#475569;font-size:12px;">Generated by NARRA AI Alert Agent • {ts}</p>
