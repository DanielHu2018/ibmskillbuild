...
const statusConfig = {
  pass: { icon: CheckCircle, color: 'text-emerald-400', bg: 'bg-emerald-500/10', border: 'border-emerald-900/30' },
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-xs text-slate-500 mt-0.5">Full audit logging, model validation, SR 11-7 compliance, and explainability layer</p>
        </div>
        <div className="flex items-center gap-2">
          <span className="rounded-full bg-emerald-500/15 px-3 py-1 text-[10px] font-semibold text-emerald-400 uppercase">
            SR 11-7 Compliant
          </span>
      ...
      <div className="grid grid-cols-4 gap-3">
        {[
          { label: 'Validation Score', value: `${passCount}/${validationChecks.length}`, color: 'text-emerald-400' },
          { label: 'Audit Entries', value: `${auditTrail.length}`, color: 'text-cyan-400' },
          { label: 'Compliance Status', value: 'PASS', color: 'text-emerald-400' },
      ...
      <div className="grid grid-cols-12 gap-4">
        ...
        <div className="col-span-7 rounded-xl border border-slate-800 bg-slate-900/50 p-5">
                ...
                <div key={i} className={cn('flex items-start gap-3 rounded-lg border px-3 py-2.5', config.bg, config.border)}>
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-[10px] font-semibold text-slate-300">{entry.component}</span>
                    </div>
                    <p className="text-[11px] text-slate-400 mt-0.5">{entry.action}</p>
                  </div>
                  <span className="shrink-0 text-[10px] font-mono text-slate-500">conf: {entry.confidence}%</span>
                </div>
              );
            })}
          </div>
          ...
          <div className="mt-4 rounded-lg border border-slate-800 bg-slate-950/50 p-3">
            <div className="text-[10px] text-slate-500 uppercase mb-2">Cycle Summary</div>
            <div className="grid grid-cols-4 gap-3 text-[11px]">
              <div><span className="text-slate-600">Total Steps: </span><span className="text-white font-mono">8</span></div>
              <div><span className="text-slate-600">Duration: </span><span className="text-white font-mono">5:42</span></div>
              <div><span className="text-slate-600">Warnings: </span><span className="text-amber-400 font-mono">1</span></div>
              <div><span className="text-slate-600">Flags: </span><span className="text-red-400 font-mono">1</span></div>
            </div>
          </div>
        </div>
        ...
        <div className="col-span-5 space-y-4">
              ...
              {validationChecks.map(c => {
                ...
                return (
                  <div key={c.check} className="flex items-center gap-2 text-[11px]">
                    ...
                    <span className="text-slate-300 flex-1">{c.check}</span>
          ...
          <div className="rounded-xl border border-slate-800 bg-slate-900/50 p-5">
            ...
            <div className="space-y-2">
              {complianceItems.map(item => (
                <div key={item.framework} className="rounded-lg border border-emerald-900/30 bg-emerald-950/10 p-3">
                  <div className="flex items-center justify-between mb-1">
                    <span className="text-xs font-medium text-white">{item.framework}</span>
                    <span className="rounded-full bg-emerald-500/15 px-2 py-0.5 text-[9px] font-bold text-emerald-400">{item.status}</span>
                  </div>
          ...
          <div className="rounded-xl border border-cyan-900/30 bg-cyan-950/10 p-5">
            <h4 className="text-xs font-semibold text-cyan-400 uppercase mb-2">Output Formats Available</h4>
            <div className="grid grid-cols-3 gap-2">
              {['PDF Report', 'JSON API', 'CSV Export'].map(fmt => (
                <button key={fmt} className="rounded-lg border border-slate-700 bg-slate-800 px-3 py-2 text-[11px] text-slate-300 hover:bg-slate-700 transition-colors cursor-pointer text-center">
                  {fmt}
                </button>
              ))}
            </div>
