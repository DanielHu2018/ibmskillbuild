...
const RISK_COLORS: Record<string, string> = {
  Low: 'text-emerald-400 bg-emerald-400/10 border-emerald-800/50',
  ...
  if (editing) return (
    <div className="flex items-center gap-1">
      <input ref={inputRef} value={draft} onChange={e => setDraft(e.target.value)}
        ...
        className="w-20 rounded bg-slate-800 border border-cyan-600 px-1.5 py-0.5 text-xs font-mono text-white focus:outline-none" type="number" />
      <button onClick={commit} className="text-emerald-400 hover:text-emerald-300"><Check size={11} /></button>
      <button onClick={() => setEditing(false)} className="text-slate-500 hover:text-red-400"><X size={11} /></button>
    </div>
  );
  return (
    <button onClick={() => { setDraft(String(value)); setEditing(true); }}
      className="group flex items-center gap-1 font-mono text-xs text-slate-300 hover:text-white transition-colors">
      {prefix}{value.toLocaleString()}
      <Edit3 size={9} className="opacity-0 group-hover:opacity-100 text-cyan-500 transition-opacity" />
    </button>
  );
}
    ...
    <div className="rounded-2xl border border-slate-700/80 bg-gradient-to-br from-slate-900 to-slate-950 p-5">
      <div className="flex items-center gap-3 mb-4">
        ...
        <div>
          ...
          <p className="text-[10px] text-slate-500">Enter ticker, shares owned, and purchase price</p>
        </div>
      </div>

      <div className="flex flex-wrap gap-1.5 mb-4">
        <span className="text-[10px] text-slate-600 self-center mr-1">Quick add:</span>
        {POPULAR_TICKERS.slice(0, 10).map(t => (
          <button key={t} onClick={() => { setSymbol(t); setSuggestion([]); }}
            className="rounded-md border border-slate-800 bg-slate-800/60 hover:border-cyan-700 hover:bg-cyan-900/20 px-2 py-0.5 text-[10px] font-mono text-slate-400 hover:text-cyan-300 transition-all">
            {t}
          </button>
        ))}
      </div>
      ...
      <div className="grid grid-cols-12 gap-3 items-end">
        <div className="col-span-2 relative">
          <label className="block text-[10px] text-slate-500 uppercase tracking-wider mb-1.5">Ticker *</label>
          <input ref={symbolRef} value={symbol} onChange={e => handleSymbolChange(e.target.value)}
            onKeyDown={e => e.key === 'Enter' && handleSubmit()}
            placeholder="AAPL" maxLength={6}
            className="w-full rounded-xl border border-slate-700 bg-slate-800 px-3 py-2.5 text-sm font-mono font-bold uppercase text-white placeholder-slate-700 focus:border-cyan-500 focus:outline-none transition-all" />
          {suggestion.length > 0 && (
            <div className="absolute top-full left-0 z-10 mt-1 w-full rounded-lg border border-slate-700 bg-slate-800 shadow-xl overflow-hidden">
              {suggestion.map(s => (
                <button key={s} onClick={() => { setSymbol(s); setSuggestion([]); }}
                  className="w-full px-3 py-2 text-left text-xs font-mono text-slate-300 hover:bg-slate-700 hover:text-white transition-colors">{s}</button>
              ))}
            </div>
          )}
        </div>
        <div className="col-span-2">
          <label className="block text-[10px] text-slate-500 uppercase tracking-wider mb-1.5">Shares *</label>
          <input value={shares} onChange={e => setShares(e.target.value)} onKeyDown={e => e.key === 'Enter' && handleSubmit()}
            ...
            className="w-full rounded-xl border border-slate-700 bg-slate-800 px-3 py-2.5 text-sm text-white placeholder-slate-700 focus:border-cyan-500 focus:outline-none transition-all" />
        </div>
        <div className="col-span-2">
          <label className="block text-[10px] text-slate-500 uppercase tracking-wider mb-1.5">Avg Buy Price ($) *</label>
          <div className="relative">
            ...
            <input value={avgCost} onChange={e => setAvgCost(e.target.value)} onKeyDown={e => e.key === 'Enter' && handleSubmit()}
              ...
              className="w-full rounded-xl border border-slate-700 bg-slate-800 pl-7 pr-3 py-2.5 text-sm text-white placeholder-slate-700 focus:border-cyan-500 focus:outline-none transition-all" />
          </div>
        </div>
        <div className="col-span-4">
          <label className="block text-[10px] text-slate-500 uppercase tracking-wider mb-1.5">Note (optional)</label>
          <input value={note} onChange={e => setNote(e.target.value)} onKeyDown={e => e.key === 'Enter' && handleSubmit()}
            ...
            className="w-full rounded-xl border border-slate-700 bg-slate-800 px-3 py-2.5 text-sm text-slate-300 placeholder-slate-700 focus:border-cyan-500 focus:outline-none transition-all" />
        </div>
        <div className="col-span-2">
          <button onClick={handleSubmit}
            className="w-full flex items-center justify-center gap-2 rounded-xl bg-cyan-600 hover:bg-cyan-500 px-4 py-2.5 text-sm font-bold text-white transition-all shadow-lg shadow-cyan-900/20">
            <Plus size={15} /> Add
          </button>
        </div>
      </div>
      ...
      <div className="mt-3 flex items-center gap-4 text-[10px] text-slate-600">
        <span className="flex items-center gap-1"><Check size={9} className="text-emerald-600" /> Enter to add quickly</span>
        <span className="flex items-center gap-1"><Save size={9} className="text-cyan-600" /> Auto-saved per portfolio</span>
        <span className="flex items-center gap-1"><Check size={9} className="text-emerald-600" /> Duplicates auto-average cost</span>
      </div>
    </div>
  );
}
...
function PortfolioTab({ portfolio, isActive, onClick, onDelete, onRename }: {
  ...
  return (
    <div className={cn(
      ...
      isActive ? 'border-cyan-600/50 bg-cyan-950/30 text-white' : 'border-slate-700 bg-slate-900/50 text-slate-400 hover:border-slate-600 hover:text-white'
    )} onClick={onClick}>
      ...
      {editing ? (
        <input ref={inputRef} value={draft} onChange={e => setDraft(e.target.value)}
          ...
          className="bg-transparent text-xs font-semibold text-white focus:outline-none w-24 border-b border-cyan-500" />
      ) : (
        <span className="text-xs font-semibold truncate max-w-[100px]">{portfolio.name}</span>
      )}
      <span className="text-[9px] text-slate-600 shrink-0">{portfolio.holdings.length}</span>
      <div className="flex items-center gap-0.5 opacity-0 group-hover:opacity-100 transition-opacity ml-1">
        <button onClick={e => { e.stopPropagation(); setEditing(true); setDraft(portfolio.name); }}
          className="rounded p-0.5 hover:bg-slate-700 hover:text-cyan-400 transition-colors"><Pencil size={9} /></button>
        <button onClick={e => { e.stopPropagation(); onDelete(); }}
          className="rounded p-0.5 hover:bg-slate-700 hover:text-red-400 transition-colors"><Trash2 size={9} /></button>
      </div>
    </div>
  );
}
      ...
      <div className="w-full max-w-md rounded-2xl border border-slate-700 bg-slate-900 p-6 shadow-2xl">
        <div className="flex items-center justify-between mb-5">
          <h3 className="text-base font-bold text-white flex items-center gap-2">
            <FolderPlus size={18} className="text-cyan-400" /> New Portfolio
          </h3>
          <button onClick={onClose} className="text-slate-500 hover:text-white transition-colors"><X size={16} /></button>
        </div>
        <div className="space-y-4">
          <div>
            <label className="block text-[10px] text-slate-500 uppercase tracking-wider mb-1.5">Portfolio Name *</label>
            <input value={name} onChange={e => setName(e.target.value)}
              ...
              className="w-full rounded-xl border border-slate-700 bg-slate-800 px-4 py-3 text-sm text-white placeholder-slate-600 focus:border-cyan-500 focus:outline-none transition-all" autoFocus />
          </div>
          <div>
            <label className="block text-[10px] text-slate-500 uppercase tracking-wider mb-1.5">Description (optional)</label>
            <input value={desc} onChange={e => setDesc(e.target.value)}
              placeholder="e.g. Long-term tech holdings, aggressive growth..."
              className="w-full rounded-xl border border-slate-700 bg-slate-800 px-4 py-3 text-sm text-slate-300 placeholder-slate-600 focus:border-cyan-500 focus:outline-none transition-all" />
          </div>
        </div>
        <div className="flex gap-3 mt-6">
          <button onClick={onClose}
            className="flex-1 rounded-xl border border-slate-700 bg-slate-800 hover:bg-slate-700 px-4 py-2.5 text-sm text-slate-300 transition-all">
            Cancel
          </button>
          <button onClick={() => { if (name.trim()) { onCreate(name.trim(), desc.trim() || undefined); onClose(); } }}
            disabled={!name.trim()}
            className="flex-1 rounded-xl bg-cyan-600 hover:bg-cyan-500 disabled:opacity-40 px-4 py-2.5 text-sm font-bold text-white transition-all">
            Create Portfolio
          </button>
        </div>
      </div>
...
function KPICard({ label, value, sub, color = 'text-white', icon: Icon, border = '' }: {
  ...
  return (
    <div className={cn('rounded-xl border bg-slate-900/60 p-4', border || 'border-slate-800')}>
...
function EmptyPortfolio({ onAdd }: { onAdd?: () => void }) {
  return (
    <div className="flex flex-col items-center justify-center py-16 rounded-2xl border border-dashed border-slate-700 bg-slate-900/30 text-center">
      <div className="mb-5 flex h-14 w-14 items-center justify-center rounded-2xl bg-slate-800 border border-slate-700">
        <Wallet size={24} className="text-slate-600" />
      </div>
      <h3 className="text-sm font-bold text-slate-400 mb-2">This portfolio is empty</h3>
      <p className="text-xs text-slate-600 max-w-xs leading-relaxed">
        Add your first position using the form above. Enter the ticker, shares owned, and what you paid per share.
      </p>
      {onAdd && (
        <button onClick={onAdd}
          className="mt-5 flex items-center gap-2 rounded-xl bg-cyan-600/20 border border-cyan-700/40 hover:bg-cyan-600/30 px-4 py-2 text-xs font-semibold text-cyan-300 transition-all">
...
function CompareView({ portfolios, getQuote }: {
  ...
  if (portfolios.length < 2) return (
    <div className="flex flex-col items-center justify-center py-20 rounded-2xl border border-dashed border-slate-700 bg-slate-900/30 text-center">
      <GitCompare size={32} className="text-slate-600 mb-4" />
      <h3 className="text-sm font-bold text-slate-400 mb-2">Need at least 2 portfolios to compare</h3>
      <p className="text-xs text-slate-600 max-w-xs">Create another portfolio using the "New Portfolio" button above, then add holdings to it.</p>
    </div>
  );
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="grid gap-4" style={{ gridTemplateColumns: `repeat(${statsAll.length}, 1fr)` }}>
        {statsAll.map(s => (
          <div key={s.id} className="rounded-2xl border border-slate-800 bg-slate-900/60 p-5 space-y-4">
            <div className="flex items-center gap-2.5">
              <div className="h-4 w-4 rounded-full" style={{ background: s.color }} />
              <h3 className="text-sm font-bold text-white">{s.name}</h3>
              <span className="text-[10px] text-slate-500">{s.liveHoldings.length} positions</span>
            </div>
            <div className="space-y-3">
              {[
                { label: 'Total Value', value: `$${s.totalValue.toLocaleString('en-US', { maximumFractionDigits: 0 })}`, color: 'text-white' },
                { label: 'Total Gain/Loss', value: `${s.totalGainLoss >= 0 ? '+' : ''}$${Math.abs(s.totalGainLoss).toLocaleString('en-US', { maximumFractionDigits: 0 })}`, color: s.totalGainLoss >= 0 ? 'text-emerald-400' : 'text-red-400' },
                { label: 'Return %', value: `${s.totalGainLossPct >= 0 ? '+' : ''}${s.totalGainLossPct.toFixed(2)}%`, color: s.totalGainLossPct >= 0 ? 'text-emerald-400' : 'text-red-400' },
                { label: 'Portfolio Beta', value: `β${s.portfolioBeta.toFixed(2)}`, color: s.portfolioBeta > 1.5 ? 'text-red-400' : s.portfolioBeta > 1 ? 'text-amber-400' : 'text-emerald-400' },
                { label: "Today's Gain", value: `${s.totalDayGain >= 0 ? '+' : ''}$${Math.abs(s.totalDayGain).toLocaleString('en-US', { maximumFractionDigits: 0 })}`, color: s.totalDayGain >= 0 ? 'text-emerald-400' : 'text-red-400' },
              ].map(m => (
      ...
      {growthData.length > 0 && (
        <div className="rounded-2xl border border-slate-800 bg-slate-900/50 p-5">
          <h3 className="text-sm font-bold text-white mb-4 flex items-center gap-2">
            <TrendingUp size={14} className="text-cyan-400" /> Estimated Portfolio Growth (10-Month Projection)
          </h3>
      ...
      {sectorChartData.length > 0 && (
        <div className="rounded-2xl border border-slate-800 bg-slate-900/50 p-5">
          <h3 className="text-sm font-bold text-white mb-4 flex items-center gap-2">
            <PieChart size={14} className="text-cyan-400" /> Sector Allocation Comparison (%)
          </h3>
          <ResponsiveContainer width="100%" height={220}>
      ...
      <div className="rounded-2xl border border-slate-800 bg-slate-900/50 p-5">
        <h3 className="text-sm font-bold text-white mb-4 flex items-center gap-2">
          <Shield size={14} className="text-cyan-400" /> Risk Metrics Comparison
        </h3>
        <div className="overflow-x-auto">
          <table className="w-full text-xs">
            <thead>
              <tr className="border-b border-slate-800">
                <th className="text-left text-[10px] text-slate-500 uppercase tracking-wider py-2 pr-4">Metric</th>
                {statsAll.map(s => (
                  <th key={s.id} className="text-right text-[10px] text-slate-400 py-2 px-3">
            ...
            <tbody className="divide-y divide-slate-800/50">
              {[
                ...
                { label: 'Cost Basis', fmt: (s: typeof statsAll[0]) => `$${s.totalCost.toLocaleString('en-US', { maximumFractionDigits: 0 })}`, colorFn: () => 'text-slate-300' },
                { label: 'Unrealized P&L', fmt: (s: typeof statsAll[0]) => `${s.totalGainLoss >= 0 ? '+' : ''}$${Math.abs(s.totalGainLoss).toLocaleString('en-US', { maximumFractionDigits: 0 })}`, colorFn: (s: typeof statsAll[0]) => s.totalGainLoss >= 0 ? 'text-emerald-400' : 'text-red-400' },
                { label: 'Return %', fmt: (s: typeof statsAll[0]) => `${s.totalGainLossPct >= 0 ? '+' : ''}${s.totalGainLossPct.toFixed(2)}%`, colorFn: (s: typeof statsAll[0]) => s.totalGainLossPct >= 0 ? 'text-emerald-400' : 'text-red-400' },
                { label: 'Portfolio Beta', fmt: (s: typeof statsAll[0]) => `β${s.portfolioBeta.toFixed(2)}`, colorFn: (s: typeof statsAll[0]) => s.portfolioBeta > 1.5 ? 'text-red-400' : s.portfolioBeta > 1 ? 'text-amber-400' : 'text-emerald-400' },
                { label: 'Positions', fmt: (s: typeof statsAll[0]) => String(s.liveHoldings.length), colorFn: () => 'text-slate-300' },
                { label: 'Today Change', fmt: (s: typeof statsAll[0]) => `${s.totalDayGain >= 0 ? '+' : ''}$${Math.abs(s.totalDayGain).toLocaleString('en-US', { maximumFractionDigits: 0 })}`, colorFn: (s: typeof statsAll[0]) => s.totalDayGain >= 0 ? 'text-emerald-400' : 'text-red-400' },
              ].map(row => (
                <tr key={row.label}>
      ...
      <div className="grid grid-cols-3 gap-4">
        {[
          ...
          <div key={card.label} className="rounded-xl border border-slate-800 bg-slate-900/50 p-4">
            <div className="text-[10px] text-slate-500 uppercase tracking-wider mb-2">{card.label}</div>
            ...
            <div className="text-lg font-black font-mono text-cyan-400 mt-1">{card.fmt(card.winner)}</div>
          </div>
        ))}
      </div>
    </div>
    ...
    <div className="space-y-5">
      ...
      <div className="flex items-start justify-between gap-4">
        <div>
          <h2 className="text-lg font-bold text-white flex items-center gap-2.5">
            <div className="flex h-8 w-8 items-center justify-center rounded-xl bg-cyan-500/10 border border-cyan-500/30">
              <Folders size={16} className="text-cyan-400" />
            </div>
            Portfolio Manager
          </h2>
        ...
        <div className="flex items-center gap-2">
          {activePortfolio && (
            <button onClick={handleRefreshAll} disabled={loadingAll}
              className="flex items-center gap-1.5 rounded-lg border border-slate-700 bg-slate-800 hover:bg-slate-700 px-3 py-2 text-xs text-slate-300 transition-colors disabled:opacity-50">
              {loadingAll ? <Loader2 size={12} className="animate-spin" /> : <RefreshCw size={12} />}
              Refresh
            </button>
          )}
          <div className="flex items-center gap-1.5 text-[10px] text-slate-600 bg-slate-900 border border-slate-800 rounded-lg px-3 py-2">
      ...
      <div className="flex items-center gap-2 flex-wrap">
        ...
        <button onClick={() => setShowNewPortfolio(true)}
          className="flex items-center gap-1.5 rounded-xl border border-dashed border-slate-700 hover:border-cyan-600 bg-slate-900/50 hover:bg-cyan-950/20 px-3 py-2 text-xs text-slate-500 hover:text-cyan-300 transition-all">
          ...
          <button onClick={() => setActiveView('compare')}
            className={cn(
              ...
              activeView === 'compare'
                ...
                : 'border-slate-700 bg-slate-900/50 text-slate-400 hover:border-purple-600/50 hover:text-purple-300'
            )}>
      ...
      {activePortfolio && activePortfolio.holdings.length > 0 && activeView !== 'compare' && (
        <div className="grid grid-cols-6 gap-3">
          ...
          <KPICard label="Cost Basis" value={`$${stats.totalCost.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`}
            sub="Invested capital" color="text-slate-300" icon={DollarSign} />
          <KPICard label="Total Gain / Loss"
            ...
            color={stats.totalGainLoss >= 0 ? 'text-emerald-400' : 'text-red-400'}
            border={stats.totalGainLoss >= 0 ? 'border-emerald-900/30' : 'border-red-900/30'}
          ...
          <KPICard label="Today's Gain"
            ...
            sub="vs. yesterday" color={stats.totalDayGain >= 0 ? 'text-emerald-400' : 'text-red-400'}
          ...
          <KPICard label="Portfolio Beta" value={stats.portfolioBeta.toFixed(2)}
            ...
            color={stats.portfolioBeta > 1.5 ? 'text-red-400' : stats.portfolioBeta > 1.0 ? 'text-amber-400' : 'text-emerald-400'}
            icon={BarChart2} />
          <KPICard label="Unrealized Return"
            ...
            color={stats.totalGainLossPct >= 0 ? 'text-emerald-400' : 'text-red-400'} icon={Target} />
        </div>
      )}
      ...
      {activePortfolio && activeView !== 'compare' && (
        <div className="flex items-center justify-between flex-wrap gap-3">
          <div className="flex gap-1 rounded-xl border border-slate-800 bg-slate-900/50 p-1 w-fit flex-wrap">
            {([
              ...
              <button key={v.key} onClick={() => setActiveView(v.key)}
                className={cn(
                  ...
                  activeView === v.key ? 'bg-slate-700 text-white shadow' : 'text-slate-500 hover:text-slate-300'
                )}>
                {v.label}
                {'count' in v && v.count !== undefined && (
                  <span className={cn('rounded-full px-1.5 py-0.5 text-[9px] font-bold', activeView === v.key ? 'bg-slate-600 text-slate-200' : 'bg-slate-800 text-slate-500')}>
                    {v.count}
                  </span>
                )}
              </button>
            ...
            <div className="flex items-center gap-2">
              <div className="relative">
                ...
                <input value={searchQuery} onChange={e => setSearchQuery(e.target.value)}
                  placeholder="Search holdings..." className="w-44 rounded-lg border border-slate-700 bg-slate-800 pl-7 pr-3 py-1.5 text-xs text-slate-300 placeholder-slate-600 focus:border-cyan-600 focus:outline-none" />
              </div>
        ...
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            ...
            <button onClick={() => setActiveView('holdings')}
              className="flex items-center gap-1.5 rounded-lg border border-slate-700 bg-slate-800 hover:bg-slate-700 px-3 py-1.5 text-xs text-slate-300 transition-colors">
      ...
      {activeView === 'holdings' && activePortfolio && (
        activePortfolio.holdings.length === 0 ? <EmptyPortfolio /> : (
          <div className="space-y-3">
            {/* Table header */}
            <div className="grid items-center px-4 py-2 text-[10px] text-slate-600 uppercase tracking-wider font-medium"
            ...
            {filtered.map(h => {
              ...
              return (
                <div key={h.id} className={cn('rounded-xl border transition-all duration-200',
                  isExpanded ? 'border-cyan-700/50 bg-slate-900' : 'border-slate-800 bg-slate-900/60 hover:border-slate-700')}>
                  ...
                  <div className="grid items-center px-4 py-3 cursor-pointer select-none"
                    ...
                    <div className="flex items-center gap-2.5" onClick={e => e.stopPropagation()}>
                      ...
                      <div className="min-w-0">
                        <div className="flex items-center gap-1.5">
                          ...
                          {isLoading && <Loader2 size={10} className="animate-spin text-cyan-400 shrink-0" />}
                        </div>
                        <div className="text-[10px] text-slate-500 truncate">{h.name}</div>
                        {h.note && <div className="text-[9px] text-slate-600 italic truncate">{h.note}</div>}
                      </div>
                    </div>
                    {/* Shares */}
                    <div className="text-right" onClick={e => e.stopPropagation()}>
                      <EditableCell value={h.shares} onSave={v => activePortfolioId && updateHolding(activePortfolioId, h.id, { shares: v })} />
                      <div className="text-[9px] text-slate-600">{weightPct.toFixed(1)}%</div>
                    </div>
                    {/* Avg Cost */}
                    <div className="text-right" onClick={e => e.stopPropagation()}>
                    ...
                    <div className="text-right">
                      ...
                      <div className="text-[9px] text-slate-600">prev ${h.previousClose.toFixed(2)}</div>
                    </div>
                    {/* Day % */}
                    <div className="text-right">
                      <div className={cn('flex items-center justify-end gap-0.5 text-xs font-mono', h.change >= 0 ? 'text-emerald-400' : 'text-red-400')}>
                        {h.change >= 0 ? <ArrowUpRight size={10} /> : <ArrowDownRight size={10} />}
                        {Math.abs(h.changePercent).toFixed(2)}%
                      </div>
                      <div className={cn('text-[9px] font-mono', h.change >= 0 ? 'text-emerald-700' : 'text-red-700')}>
                    ...
                    {/* Gain/Loss */}
                    <div className="text-right">
                      <div className={cn('text-xs font-mono font-bold', h.gainLoss >= 0 ? 'text-emerald-400' : 'text-red-400')}>
                        {h.gainLoss >= 0 ? '+' : '-'}${Math.abs(h.gainLoss).toLocaleString('en-US', { maximumFractionDigits: 0 })}
                      </div>
                      <div className={cn('text-[10px] font-mono font-semibold', h.gainLoss >= 0 ? 'text-emerald-600' : 'text-red-600')}>
                    ...
                    <div className="text-right" onClick={e => e.stopPropagation()}>
                      ...
                      <div className="text-[9px] text-slate-600 mt-0.5">β={h.beta}</div>
                    </div>
                    {/* Action */}
                    <div className="flex items-center justify-end gap-1" onClick={e => e.stopPropagation()}>
                      ...
                      <button onClick={() => setExpandedId(isExpanded ? null : h.id)}
                        className={cn('p-1 rounded transition-colors', isExpanded ? 'text-cyan-400' : 'text-slate-600 hover:text-cyan-400')}>
                        <Eye size={13} />
                      </button>
                  ...
                  {/* Expanded detail */}
                  {isExpanded && (
                    <div className="border-t border-slate-800 px-5 py-5 grid grid-cols-3 gap-6">
                      {/* Price ladder */}
                      <div>
                        ...
                        <div className="space-y-2.5">
                          {[
                            { label: 'Bull Case', price: h.bullTarget, color: 'bg-emerald-500', textColor: 'text-emerald-300', note: 'Momentum extension' },
                            { label: 'Base Sell Target', price: h.sellTarget, color: 'bg-amber-500', textColor: 'text-amber-300', note: 'Primary exit · NARRA model' },
                            { label: 'Conservative', price: h.conservativeTarget, color: 'bg-yellow-600', textColor: 'text-yellow-400', note: 'Minimum exit' },
                            { label: 'Current Price', price: h.currentPrice, color: 'bg-cyan-500', textColor: 'text-cyan-300', note: 'Live market', current: true },
                            ...
                            <div key={i} className={cn('flex items-center gap-3 rounded-lg px-3 py-2', item.current ? 'bg-cyan-950/30 border border-cyan-900/40' : 'bg-slate-800/40')}>
                              ...
                              <div className="flex-1 min-w-0">
                                ...
                                <div className="text-[9px] text-slate-600 truncate">{item.note}</div>
                              </div>
                              <div className={cn('text-sm font-bold font-mono shrink-0', item.textColor)}>${item.price.toFixed(2)}</div>
                              {!item.current && (
                                <div className="text-[9px] font-mono text-slate-600 shrink-0 w-14 text-right">
                      ...
                      <div>
                        <div className="text-xs font-bold text-white mb-4 flex items-center gap-2">
                          <div className="h-5 w-5 rounded-lg bg-cyan-500/10 border border-cyan-500/30 flex items-center justify-center">
                        ...
                        <div className="grid grid-cols-2 gap-2">
                          {[
                            ...
                            <div key={s.label} className="rounded-lg bg-slate-800/50 p-2.5">
                              <div className="text-[9px] text-slate-500 uppercase tracking-wider mb-1">{s.label}</div>
                              <div className="text-xs font-mono font-bold text-slate-200">{s.value}</div>
                              <div className="text-[9px] text-slate-600">{s.note}</div>
                            </div>
                          ))}
                        </div>
                      </div>
                      ...
                      <div>
                        ...
                        <div className={cn('flex items-center gap-2 rounded-xl border px-4 py-3 mb-4',
                          h.recommendation === 'Strong Hold' ? 'border-emerald-800/50 bg-emerald-950/30' :
                          h.recommendation === 'Hold' ? 'border-cyan-800/50 bg-cyan-950/20' :
                          ...
                          <div>
                            ...
                            <div className="text-[10px] text-slate-500">NARRA regime-adjusted signal</div>
                          </div>
                        </div>
                        <div className="rounded-xl bg-slate-800/50 p-3.5 text-[11px] text-slate-400 leading-relaxed mb-3">
                          {h.recommendation === 'Strong Hold' && <>Position is up <span className="text-emerald-400 font-semibold">{h.gainLossPct.toFixed(1)}%</span>. Momentum supports upside. Raise trailing stop to ${h.trailingStop.toFixed(2)} to lock gains. Consider partial exit near ${h.sellTarget.toFixed(2)} or hold for bull case ${h.bullTarget.toFixed(2)}.</>}
                          {h.recommendation === 'Hold' && <>Position shows <span className={h.gainLoss >= 0 ? 'text-emerald-400 font-semibold' : 'text-amber-400 font-semibold'}>{h.gainLossPct >= 0 ? '+' : ''}{h.gainLossPct.toFixed(1)}%</span>. Maintain position. Hard stop at ${h.stopLoss.toFixed(2)}. Primary target ${h.sellTarget.toFixed(2)}.</>}
                          {h.recommendation === 'Reduce' && <>Consider trimming 25-50% of position. {h.gainLossPct > 0 ? <>Lock in <span className="text-emerald-400 font-semibold">{h.gainLossPct.toFixed(1)}% gains</span>.</> : <>Down <span className="text-red-400 font-semibold">{Math.abs(h.gainLossPct).toFixed(1)}%</span> — limit drawdown.</>} Hard stop ${h.stopLoss.toFixed(2)}.</>}
                        ...
                        <div className="flex flex-wrap gap-1.5">
                          <span className="text-[9px] rounded-md bg-slate-800 border border-slate-700 px-2 py-0.5 text-slate-500">β={h.beta}</span>
                          <span className="text-[9px] rounded-md bg-slate-800 border border-slate-700 px-2 py-0.5 text-slate-500">P/E {h.pe}x</span>
                          <span className="text-[9px] rounded-md bg-slate-800 border border-slate-700 px-2 py-0.5 text-slate-500">{h.sector}</span>
            ...
            {filtered.length === 0 && searchQuery && (
              <div className="flex items-center justify-center py-12 text-slate-600 text-sm">No holdings match "{searchQuery}"</div>
            )}

            <div className="rounded-xl border border-slate-800 bg-slate-900/40 p-4 flex items-start gap-3">
              <AlertTriangle size={14} className="text-amber-400 mt-0.5 shrink-0" />
              <div className="text-xs text-slate-400 leading-relaxed">
                <span className="text-amber-400 font-bold">NARRA Advisory: </span>
        ...
        <div className="space-y-5">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h3 className="text-sm font-bold text-white">NARRA Top Buy Recommendations</h3>
                <p className="text-[10px] text-slate-500">AI-generated · Fundamentals + momentum + regime factors</p>
              </div>
            </div>
          ...
          <div className="grid grid-cols-2 gap-4">
            {recommendations.map((rec, idx) => (
              <div key={rec.symbol} className={cn('rounded-2xl border p-5 space-y-4 hover:border-slate-600 transition-all',
                rec.rating === 'Strong Buy' ? 'border-emerald-800/50 bg-gradient-to-br from-emerald-950/20 to-slate-900' :
                rec.rating === 'Buy' ? 'border-cyan-800/30 bg-gradient-to-br from-cyan-950/10 to-slate-900' :
                'border-slate-800 bg-slate-900/50')}>
                <div className="flex items-start justify-between">
                  <div className="flex items-center gap-3">
                    <div className="flex h-11 w-11 items-center justify-center rounded-xl bg-slate-800 border border-slate-700 text-sm font-bold font-mono text-cyan-400 relative">
                      {rec.symbol.slice(0, 2)}
                      <div className="absolute -top-1.5 -right-1.5 text-[9px] font-black bg-slate-900 rounded-full px-1 border border-slate-700 text-slate-400">#{idx + 1}</div>
                    </div>
                    <div>
                      <div className="flex items-center gap-2">
                        <span className="text-sm font-bold text-white font-mono">{rec.symbol}</span>
                        <span className={cn('text-[10px] font-black px-2 py-0.5 rounded-full',
                          rec.rating === 'Strong Buy' ? 'bg-emerald-500/20 text-emerald-400 border border-emerald-700/40' :
                          rec.rating === 'Buy' ? 'bg-cyan-500/20 text-cyan-400 border border-cyan-700/40' :
                          'bg-amber-500/20 text-amber-400 border border-amber-700/40')}>{rec.rating}</span>
                      </div>
                      <div className="text-xs text-slate-400 mt-0.5">{rec.name}</div>
                      <div className="text-[10px] text-slate-600">{rec.sector}</div>
                    </div>
                  </div>
                  <div className="text-right">
                    <div className="text-xl font-black font-mono text-white">${rec.currentPrice.toFixed(2)}</div>
                    <div className={cn('text-xs font-mono flex items-center justify-end gap-0.5', rec.changePercent >= 0 ? 'text-emerald-400' : 'text-red-400')}>
                ...
                <div className="grid grid-cols-4 gap-2">
                  {[
                    ...
                    { label: 'Upside', value: `+${rec.upside.toFixed(1)}%`, color: 'text-emerald-400' },
                    { label: 'P/E', value: `${rec.pe}x`, color: 'text-slate-300' },
                    { label: 'Beta', value: `β${rec.beta}`, color: 'text-slate-300' },
                  ].map(m => (
                    <div key={m.label} className="rounded-lg bg-slate-800/60 px-2.5 py-2 text-center">
                ...
                <div>
                  ...
                  <div className="h-2 rounded-full bg-slate-800 overflow-hidden">
                    <div className="h-full rounded-full bg-gradient-to-r from-cyan-600 to-emerald-500" style={{ width: `${Math.min((rec.upside / 40) * 100, 100)}%` }} />
                  </div>
                </div>
                <p className="text-[11px] text-slate-400 leading-relaxed">{rec.reason}</p>
                <div className="flex items-center justify-between pt-1 border-t border-slate-800">
                  <div className="flex gap-1.5">
                    <span className="text-[9px] rounded-md bg-slate-800 px-2 py-0.5 text-slate-500 border border-slate-700">{rec.sector}</span>
                  </div>
                  <button onClick={() => setActiveView('holdings')}
                    className="flex items-center gap-1.5 rounded-lg bg-slate-800 hover:bg-slate-700 border border-slate-700 hover:border-cyan-700 px-3 py-1.5 text-[10px] font-semibold text-slate-300 hover:text-cyan-300 transition-all">
          ...
          <div className="rounded-xl border border-amber-800/30 bg-amber-950/10 p-4 flex items-start gap-3">
            ...
            <p className="text-xs text-amber-400/80 leading-relaxed">
              <strong>Disclaimer:</strong> NARRA recommendations are AI-generated for informational purposes only and do not constitute investment advice. All investments carry risk. Past performance is not indicative of future results. Please consult a licensed financial advisor.
            </p>
          </div>
        </div>
      )}
          ...
          <div className="space-y-4">
            <div className="grid grid-cols-2 gap-4">
              <div className="rounded-xl border border-slate-800 bg-slate-900/50 p-5">
                <div className="flex items-center gap-2 mb-4"><BarChart2 size={14} className="text-cyan-400" /><h3 className="text-sm font-semibold text-white">Unrealized P&L by Position</h3></div>
              ...
              <div className="rounded-xl border border-slate-800 bg-slate-900/50 p-5">
                <div className="flex items-center gap-2 mb-4"><PieChart size={14} className="text-cyan-400" /><h3 className="text-sm font-semibold text-white">Sector Allocation</h3></div>
            ...
            <div className="rounded-xl border border-slate-800 bg-slate-900/50 p-5">
              <div className="flex items-center gap-2 mb-4"><Database size={14} className="text-cyan-400" /><h3 className="text-sm font-semibold text-white">Position Weight Breakdown</h3></div>
              <div className="space-y-2.5">
                    ...
                    <div key={h.id} className="space-y-1.5">
                      <div className="flex items-center justify-between text-xs">
                        <div className="flex items-center gap-3">
                          ...
                          <span className="text-slate-500 hidden sm:block">{h.name}</span>
                        </div>
                        <div className="flex items-center gap-6 font-mono">
                          <span className="text-slate-300 w-20 text-right">${h.marketValue.toLocaleString('en-US', { maximumFractionDigits: 0 })}</span>
                          <span className={cn('w-20 text-right', h.gainLoss >= 0 ? 'text-emerald-400' : 'text-red-400')}>
                            {h.gainLoss >= 0 ? '+' : '-'}${Math.abs(h.gainLoss).toLocaleString('en-US', { maximumFractionDigits: 0 })}
                          </span>
                          <span className={cn('w-14 text-right font-semibold', h.gainLossPct >= 0 ? 'text-emerald-400' : 'text-red-400')}>
                            {h.gainLossPct >= 0 ? '+' : ''}{h.gainLossPct.toFixed(1)}%
                          </span>
                          <span className="text-slate-500 w-10 text-right">{weight.toFixed(1)}%</span>
                        </div>
                      </div>
            ...
            <div className="grid grid-cols-3 gap-4">
              {[
                ...
                <div key={card.label} className="rounded-xl border border-slate-800 bg-slate-900/50 p-5">
                  <div className="flex items-center gap-2 mb-3"><card.icon size={13} className="text-slate-500" /><div className="text-[10px] text-slate-500 uppercase tracking-wider">{card.label}</div></div>
                  {card.value ? (
                    <>
                      <div className="text-2xl font-black text-white font-mono mb-1">{card.value.symbol}</div>
                      <div className="text-sm font-semibold font-mono text-cyan-400">{card.fmt(card.value)}</div>
                    </>
