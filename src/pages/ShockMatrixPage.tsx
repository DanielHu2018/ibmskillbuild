    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-xs text-slate-500 mt-0.5">Regime-aware shock generation with historical analog grounding — IBM Granite LLM</p>
        </div>
        <div className="flex items-center gap-3">
          ...
          <span className="rounded-full bg-emerald-500/15 px-3 py-1 text-[10px] font-semibold text-emerald-400 uppercase">Analog Grounded</span>
        </div>
      </div>
      ...
      <div className="grid grid-cols-4 gap-3">
        {[
          { label: 'Factors Shocked', value: '8', color: 'text-cyan-400' },
          { label: 'Avg Confidence', value: `${Math.round(shockMatrix.reduce((s, x) => s + x.confidence, 0) / shockMatrix.length)}%`, color: 'text-emerald-400' },
      ...
      <div className="grid grid-cols-12 gap-4">
        {/* Full Matrix Table */}
        <div className="col-span-8 rounded-xl border border-slate-800 bg-slate-900/50 p-5">
              ...
              {shockMatrix.map((s) => (
                <tr
                  ...
                  className={cn(
                    ...
                    selectedFactor === s.factor ? 'bg-cyan-950/20' : 'hover:bg-slate-800/30'
                  )}
                >
                  ...
                  <td className="py-2.5 text-right">
                    <div className="flex items-center justify-end gap-2">
                      <div className="h-1.5 w-16 rounded-full bg-slate-800">
                        <div
                          className={cn('h-full rounded-full', s.confidence >= 80 ? 'bg-emerald-500' : s.confidence >= 65 ? 'bg-amber-500' : 'bg-red-500')}
                          style={{ width: `${s.confidence}%` }}
                        />
                      </div>
                      <span className="font-mono text-slate-400 w-8 text-right">{s.confidence}%</span>
                    </div>
                  </td>
                  <td className="py-2.5 text-right font-mono text-slate-500">{(s.probability * 100).toFixed(0)}%</td>
                  <td className="py-2.5 pl-3 text-slate-500 text-[11px]">{s.analogRef}</td>
                </tr>
              ))}
            </tbody>
          </table>
            ...
            <>
                ...
                <div className="space-y-3">
                  {[
                    ...
                    <div key={scenario.label} className={cn('rounded-lg border p-3', `border-${scenario.color}-900/30 bg-${scenario.color}-950/10`)}>
                      <div className="flex items-center justify-between">
                        <span className="text-[10px] text-slate-400 uppercase">{scenario.label}</span>
              ...
              <div className="rounded-xl border border-slate-800 bg-slate-900/50 p-5">
                ...
                <div className="rounded-lg border border-slate-800 bg-slate-950/50 p-3">
                  <div className="text-xs font-medium text-cyan-400">{selected.analogRef}</div>
                  <div className="text-[10px] text-slate-500 mt-1">Historical analog used for shock calibration</div>
