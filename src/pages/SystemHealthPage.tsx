    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-xs text-slate-500 mt-0.5">Real-time monitoring of all NARRA subsystems, latency metrics, and pipeline performance</p>
        </div>
      ...
      <div className="grid grid-cols-6 gap-3">
        {[
          ...
          { label: 'Events Processed', value: systemMetrics.eventsProcessed.toLocaleString(), color: 'text-cyan-400' },
          { label: 'Avg Latency', value: systemMetrics.avgLatency, color: 'text-emerald-400' },
            ...
            {services.map(s => (
              <div key={s.name} className="flex items-center justify-between rounded-lg border border-slate-800 bg-slate-950/40 px-3 py-2.5">
                ...
                <div className="flex items-center gap-4">
                  <span className="text-[10px] font-mono text-slate-500">Latency: {s.latency}</span>
                  <span className="text-[10px] font-mono text-slate-500">Uptime: {s.uptime}</span>
        ...
        <div className="col-span-5 space-y-4">
          <div className="rounded-xl border border-slate-800 bg-slate-900/50 p-5">
            ...
            <div className="space-y-3">
              {pipelineSteps.map(p => (
                <div key={p.step} className="rounded-lg border border-emerald-900/30 bg-emerald-950/10 p-3">
                  <div className="flex items-center justify-between mb-1">
                    <span className="text-xs text-slate-200 font-medium">{p.step}</span>
                    <CheckCircle size={13} className="text-emerald-400" />
                  </div>
                  <div className="flex items-center gap-2 text-[10px]">
                    <span className="text-slate-500">Target: {p.target}</span>
                    <span className="text-slate-700">→</span>
                    <span className="text-emerald-400 font-mono font-semibold">Actual: {p.actual}</span>
                  </div>
          ...
          <div className="rounded-xl border border-slate-800 bg-slate-900/50 p-5">
            ...
            <div className="space-y-2 text-[11px]">
              {[
                ...
                <div key={p.param} className="flex items-center justify-between border-b border-slate-800/50 pb-1.5">
                  <span className="text-slate-500">{p.param}</span>
                  <span className="font-mono text-cyan-400">{p.value}</span>
                </div>
              ))}
            </div>
          </div>
          ...
          <tbody>
            {recentCycles.map(c => (
              <tr key={c.id} className="border-b border-slate-800/50 hover:bg-slate-800/30 transition-colors">
                <td className="py-2.5 font-mono text-cyan-400">{c.id}</td>
                <td className="py-2.5 text-right font-mono text-slate-400">{c.time}</td>
                <td className="py-2.5 text-right font-mono text-slate-300">{c.duration}</td>
                <td className="py-2.5 text-right font-mono text-slate-400">{c.events}</td>
                <td className="py-2.5 text-right font-mono text-slate-400">{c.shocks}</td>
                <td className={cn('py-2.5 text-right font-mono', c.breaches > 0 ? 'text-red-400 font-bold' : 'text-slate-500')}>{c.breaches}</td>
                <td className="py-2.5 text-right">
                  <span className="rounded-full bg-emerald-500/15 px-2 py-0.5 text-[9px] font-semibold text-emerald-400 uppercase">{c.status}</span>
                </td>
              </tr>
            ))}
          </tbody>
